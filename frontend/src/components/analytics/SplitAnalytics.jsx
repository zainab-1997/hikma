...
function SplitTable({ rows, type }) {
  ...
  return <div className="analytics-table-wrap"><table className="analytics-table">
    ...
    <tbody>{rows.map((row) => <tr key={row[nameKey]}>
      <td>{type === 'price' ? priceLabel(row[nameKey]) : row[nameKey]}</td>
      <td>{number.format(row.order_count)}</td><td>{number.format(row.sales_total)} IQD</td>
      {type === 'price' && <td>{row.percentage_of_total_sales}%</td>}
    </tr>)}</tbody>
  </table></div>
}
